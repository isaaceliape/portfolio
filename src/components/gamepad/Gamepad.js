...
class Gamepad {
  ...
  gameLoop() {
    ...
    gamepads = gamepads ? navigator.webkitGetGamepads : [];
    ...
    if (buttonPressed(gp.buttons[14])) {
      state.cursor.position.x =
        (state.cursor.position.x - 10) < 0
          ? 0
          : state.cursor.position.x - 10;
    ...
    if (buttonPressed(gp.buttons[15])) {
      state.cursor.position.x =
        (state.cursor.position.x + 10) > window.innerWidth
          ? window.innerWidth
          : state.cursor.position.x + 10;
      // console.log('state.cursor.position.x', state.cursor.position.x);
    }
    ...
    if (buttonPressed(gp.buttons[12])) {
      state.cursor.position.y =
        (state.cursor.position.y - 10) < 0
          ? 0
          : state.cursor.position.y - 10;
    }
    // console.log('down', state.cursor.position);
    if (buttonPressed(gp.buttons[13])) {
      state.cursor.position.y =
        (state.cursor.position.y + 10) > window.innerHeight
          ? window.innerHeight
          : state.cursor.position.y + 10;
    }
    this.setState(state, () => {
